...
func TestLocker(t *testing.T) {
	...
	isLocked, err := locker.IsLocked(ctx, client)
	assert.NoError(t, err)
	assert.False(t, isLocked)

	// grab lock
	lock1, err := locker.Grab(ctx, client, time.Second)
	assert.NoError(t, err)
	...
	lock2, err := locker.Grab(ctx, client, time.Second)
	assert.NoError(t, err)
	assert.Zero(t, lock2)

	// should succeed if we wait longer
	lock3, err := locker.Grab(ctx, client, time.Second*6)
	assert.NoError(t, err)
	assert.NotZero(t, lock3)
	assert.NotEqual(t, lock1, lock3)

	// extend the lock
	err = locker.Extend(ctx, client, lock3, time.Second*10)
	assert.NoError(t, err)

	// trying to grab it should fail with a 5 second timeout
	lock4, err := locker.Grab(ctx, client, time.Second*5)
	assert.NoError(t, err)
	assert.Zero(t, lock4)

	// try to release the lock with wrong value
	err = locker.Release(ctx, client, "2352")
	assert.NoError(t, err)
	...
	// release the lock
	err = locker.Release(ctx, client, lock3)
	assert.NoError(t, err)

	assertredis.NotExists(t, client, "test")

	// new grab should work
	lock5, err := locker.Grab(ctx, client, time.Second*5)
	assert.NoError(t, err)
