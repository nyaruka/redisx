...
func TestNewPool(t *testing.T) {
	// check defaults
	rp, err := redisx.NewPool("redis://valkey8:6379/15")
	assert.NoError(t, err)
	assert.Equal(t, 32, rp.MaxActive)
	assert.Equal(t, 4, rp.MaxIdle)
	assert.Equal(t, 180*time.Second, rp.IdleTimeout)

	rp, err = redisx.NewPool("redis://valkey8:6379/15", redisx.WithMaxActive(10), redisx.WithMaxIdle(3), redisx.WithIdleTimeout(time.Minute))
	assert.NoError(t, err)
